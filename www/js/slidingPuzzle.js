(function(angular) {
    ...
    var module = angular.module('slidingPuzzle', ['LocalStorageModule']);

    module.factory('ModalService', function($ionicModal, $rootScope) {
     ...
     * Service
     */
    module.factory('slidingPuzzle', function(CacheService) {
        function shuffle(a) {
            var q;
        ...
        function SlidingPuzzle(rows, cols) {
            ...
            this.move = function(srow, scol) {
                ...
                if(this.isSolved() == true) {
                    // if it is solved
                    var gotLevel = CacheService.get('gotLevel'),
                        level = CacheService.get('level');

                    if( (level < this.levelmax) && (level == gotLevel) ) {
                        var levelUp = parseInt(level) + parseInt(1);
                        CacheService.set('gotLevel', levelUp);        
                    }
                    this.showPopup();
                }
            ...
            this.showPopup = function() {
                document.getElementById('finishscr').style.display = "block";
            }
            /**
             * Is solved?
             * @type {Boolean}
        ...
        return {
            ...
            link: function(scope, element, attrs) {
                ...
                function tile() {
                    ...
                    //check if image width larger than window width
